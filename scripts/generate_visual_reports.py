...
def create_performance_comparison_chart(df):
    ...
    if "model_type" in df.columns:
        grouped = (
            df.groupby("model_type")["accuracy"].max().sort_values(ascending=False)
        )
    elif "n_estimators" in df.columns:
        # For random forest experiments
        grouped = (
            df.groupby("n_estimators")["accuracy"].max().sort_values(ascending=False)
        )
    else:
...
def generate_automated_report():
    ...
    if cm_fig:
        cm_fig.savefig(
            figures_dir / "confusion_matrix.png", dpi=300, bbox_inches="tight"
        )
    ...
    print("Summary saved to reports/auto_report_summary.json")
